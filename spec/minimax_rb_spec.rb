...
RSpec.describe MinimaxRB do
  describe "tic-tac-toe board" do
    ...
    it "returns false for a not full board" do
      board = ["X", "O", "X", nil, "X", "O", "O", "X", "O"]
      empty_marker = nil

      expect(MinimaxRB.board_full?(board, empty_marker)).to eq false
    end
  ...
  describe "game rules" do
    ...
    it "checks if a winning combo contains a specific marker, returns false" do
      marker_o = "O"
      o_winning_combo = ["O", "O", "X"]

      expect(MinimaxRB.marker_winning_combo?(marker_o, o_winning_combo)).to eq false
    end

    it "returns true if a specific marker won" do
      marker_x = "X"
    ...
    it "returns false if no specific marker won" do
      marker_x = "X"
      x_winning_board = ["X", nil, nil, "X", nil, nil, nil, nil, nil]

      expect(MinimaxRB.marker_winner?(marker_x, x_winning_board)).to eq false
    end
    ...
    it "returns false for not a draw" do
      board = ["X", "O", "X", "O", "X", "O", "O", "X", nil]
      marker = "X"
      opponent = "O"
      empty_marker = nil

      expect(MinimaxRB.draw?(board, marker, opponent, empty_marker)).to eq false
    end
    ...
    it "returns false for not a win" do
      board = ["X", "X", nil, "O", "O", nil, nil, nil, nil]
      marker = "X"
      opponent = "O"

      expect(MinimaxRB.win?(board, marker, opponent)).to eq false
    end
  end
  ...
  describe "best move" do
    ...
    it "returns an error if one or more of the arguments is missing" do
      args = {
        board: "board",
        opponent_marker: "O"
      }
